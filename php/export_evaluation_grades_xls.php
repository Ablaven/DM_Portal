...
try {
    ...
    $displayItems = array_values(array_filter(
        $itemsOut,
        static fn(array $item): bool => (string)($item['category'] ?? '') !== 'attendance'
    ));

    $header = ['Student ID', 'Student'];
    foreach ($displayItems as $item) {
        $label = trim((string)$item['label']);
        $header[] = $label !== '' ? $label : 'Item';
    }
    $header[] = 'Attendance';
    $header[] = 'Final';
    ...
    $colWidths = array_fill(0, count($header), 12);
    $colWidths[0] = 16;
    $colWidths[1] = 40;

    $scoreColCount = count($displayItems);
    $scoreStartIndex = 2;
    for ($i = 0; $i < $scoreColCount; $i++) {
        $labelLen = mb_strlen((string)($displayItems[$i]['label'] ?? ''));
        $colWidths[$scoreStartIndex + $i] = $labelLen >= 14 ? 16 : 12;
    }

    $attendanceIndex = $scoreStartIndex + $scoreColCount;
    $finalIndex = $attendanceIndex + 1;
    $colWidths[$attendanceIndex] = 12;
    $colWidths[$finalIndex] = 10;
    ...
    foreach ($students as $s) {
        ...
        foreach ($displayItems as $item) {
            ...
            $row[] = isset($scores[$itemId]) ? number_format((float)$scores[$itemId], 2) : '';
        }

        $row[] = number_format($attendanceScore, 2);
        $row[] = $finalScore !== null ? number_format($finalScore, 2) : '';
