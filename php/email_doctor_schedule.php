...
try {
    ...
    $dStmt = $pdo->prepare('SELECT doctor_id, full_name, email, color_code FROM doctors WHERE doctor_id = :id');
    ...
    $docEmail = trim((string)($doctor['email'] ?? ''));
    if ($docEmail === '') {
        http_response_code(400);
        echo json_encode(['success' => false, 'error' => 'Doctor email missing']);
        exit;
    }
